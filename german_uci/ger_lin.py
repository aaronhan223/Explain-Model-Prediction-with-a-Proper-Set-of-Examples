...
# load dataset
# dataset = np.genfromtxt('german.data-numeric')
# data_X = dataset[:, :-1]
# data_y = dataset[:, -1]
# data_y = data_y - 1
...
# y_train = np.eye(2)[y_train.astype(np.int)]
    ...
    for tt in range(50):
       ...
       #  nn_x = fine_marginline(logit)
        ...
        if  ( 0.000 + best_loss )>= cur_loss :
            ...
            np.savetxt('lin_select.csv', data, fmt='%d', delimiter=',', header=hd_str)
            break
